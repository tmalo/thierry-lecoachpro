...
function parseTestimonialContent(content: string): Testimonial["content"] {
  ...
  parts.forEach((part) => {
    ...
    if (title.includes("avant le coaching")) {
      sections.avant = text
    } else if (title.includes("difficultés au quotidien") || title.includes("difficultes au quotidien")) {
      sections.difficultes = text
    } else if (title.includes("ce que le coaching m'a apporté") || title.includes("ce que le coaching m'a apporte")) {
      sections.apport = text
    } else if (title.includes("résultats concrets") || title.includes("resultats concrets")) {
...
export async function getTestimonialsByOffre(offre: string): Promise<Testimonial[]> {
  const testimonials = await getTestimonialsWithCache()
